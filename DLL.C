...
do{
if(link->data==data){
printf("\\n\t\tThe number is found int the linked list!! %d and position is %d",link->data,count);
return;
}
count++;
link=link->next;
...
while(link!=NULL);
printf("\n\t\tThe number is found in the LL!!");
}
}
int main(){
int ch,data,pos;
do{
printf("\nChoose:\n1.Insert head\n2.Insert end\n3.Insert at particulur position\n4.Delete head\n5.Delete end\n6.Delete from particulur position\n7.Display Forward\n8.Display reverse\n9.Search\n10.End\n::");
scanf("%d",&ch);
switch(ch){
case 1:
...
printf("Enter the data you need to store :");
scanf("%d",&data);
printf("Enter the positon where you need to store(Head=0:End=%d):",size);
scanf("%d",&pos);
if(pos<0||pos>size)
printf("\n\t\tInvalid POsition!!");
else if(pos==0)
